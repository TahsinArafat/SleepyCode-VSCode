...
public struct SleepyCodePropertyKey {
    public Guid fmtid;
    public uint pid;
    public SleepyCodePropertyKey(Guid fmtid, uint pid) { this.fmtid = fmtid; this.pid = pid; }
}

[StructLayout(LayoutKind.Explicit)]
public struct SleepyCodePropVariant {
...
public interface ISleepyCodePropertyStore {
    [PreserveSig] int GetCount(out uint cProps);
    [PreserveSig] int GetAt(uint iProp, out SleepyCodePropertyKey pkey);
    [PreserveSig] int GetValue(ref SleepyCodePropertyKey key, out SleepyCodePropVariant pv);
    [PreserveSig] int SetValue(ref SleepyCodePropertyKey key, ref SleepyCodePropVariant pv);
    [PreserveSig] int Commit();
}

public static class SleepyCodeShortcut {
    ...
    private static extern int SHGetPropertyStoreFromParsingName(
        string pszPath, IntPtr pbc, int flags, ref Guid riid, out ISleepyCodePropertyStore ppv);

    public static void SetAppUserModelId(string shortcutPath, string appId) {
        ISleepyCodePropertyStore store;
        Guid iid = typeof(ISleepyCodePropertyStore).GUID;
        ...
        SleepyCodePropVariant value = new SleepyCodePropVariant { vt = 31, pointerValue = Marshal.StringToCoTaskMemUni(appId) };
        try {
...
try {
    ...
    $lnkPath = Join-Path $startMenu 'SleepyCode.lnk'
    ...
    $shortcut.Description = 'SleepyCode notifications'
    if ($iconPath -and (Test-Path $iconPath)) {
        $shortcut.IconLocation = "$iconPath,0"
    }
    $shortcut.Save()
    [SleepyCodeShortcut]::SetAppUserModelId($lnkPath, $appId)
