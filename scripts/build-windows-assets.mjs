...
const sourcePng = join(root, 'media', 'sleepycode-notification.png');
const outDir = join(root, 'native', 'windows');
const outIco = join(outDir, 'sleepycode.ico');

mkdirSync(outDir, { recursive: true });

const resized = join(tmpdir(), `sleepycode-icon-${Date.now()}.png`);
try {
