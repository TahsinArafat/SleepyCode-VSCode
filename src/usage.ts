...
const USAGE_STORAGE_KEY = 'sleepycode.usage';
...
export function aggregateUsage(records: UsageRecord[]): UsageAggregate {
  ...
  const overallAvgTokensPerSecond = totalRequests > 0 ? Math.round((totalTokensForSpeed / totalDurationMs) * 1000) : 0;
  return { models, totals, overallAvgTokensPerSecond, totalRequests, totalDurationMs };
}
